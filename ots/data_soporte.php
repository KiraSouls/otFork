...
if ($row == '1') {
    $sel = $con->query("
SELECT c.id, c.soporte_remoto, c.estado, c.created_at,
br.id_servicio
FROM  convenios c
...
");
    while ($f = $sel->fetch_assoc()) {
        $val = $f['soporte_remoto'];
    }

    $sel2 = $con->query("SELECT count(*) FROM ots where MONTH(created_at) = $periodo_mes and YEAR(created_at) = $periodo_año and tipo_visitas = 'Soporte Remoto'
