...
if ($row == '1') {
    $sel = $con->query("
SELECT c.id, c.visitas_presenciales, c.estado, c.created_at,
br.id_servicio
FROM  convenios c
...
");
    while ($f = $sel->fetch_assoc()) {
        $val = $f['visitas_presenciales'];
    }

    $sel2 = $con->query("SELECT count(*) FROM ots where MONTH(created_at) = $periodo_mes and YEAR(created_at) = $periodo_año and tipo_visitas = 'Presencial'
