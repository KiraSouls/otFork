...
?>
   ...
   <div class="row">
      ...
      <?php
                     ...
                     while ($f = $sel2->fetch_assoc()) {  ?>
                  <option value='<?php echo $f['id'] ?>' <?php if($f['id']==$id_service) { echo 'selected'; } ?> > <?php echo $f['service_name'] ?></option>
               <?php  } ?>
               </select>
      </div>
...
<?php
       ...
       ?>
         <table class="highlight striped" id="dynamic_field">
          ...
          while ($m = $sel_sublines->fetch_assoc()) { ?>
            <option value='<?php echo $m['id'] ?>' <?php if($m['id']==$sublinea_id) { echo 'selected'; } ?> ><?php echo $m['name'] ?></option>
            <?php   } ?>
                     </select>
               </div>
              </td>
                  ...
                  <select style="height:150px;width:300px;" class="browser-default" name="task<?php echo $dataset ?>[]" id="task<?php echo $dataset ?>" multiple>
                    ...
                    $id_equip =$k['id'];
                                   ...
                                   while ($p = $sel2->fetch_assoc()) {
                                    $id_p = $p['id']; 
                                  ?>
                                <option  value='<?php echo $p['id'] ?>' <?php  $sel_s =  $con->query("SELECT * FROM tasks_equipments WHERE number_ot='$number' AND id_equipment='$id_equip' AND id_tasks='$id_p'"); $filas = mysqli_num_rows($sel_s);  if ($filas > 0) { echo selected; }  ?> > <?php echo $p['name'] ?></option>
      ...
      <?php
                       ...
                       INNER JOIN techs tech ON tech.id= ts.id_tech");
                     while ($f = $sel2->fetch_assoc()) {  ?>
                  <option value='<?php echo $f['name'] ?>' <?php if($f['name']==$leader) { echo 'selected'; } ?> > <?php echo $f['name'] ?></option>
               <?php  } ?>
               </select>
      </div>
       </div>
      ...
      <div class = "col s6">
                  ...
                  <select class="" name="priority" id="priority" >
                        ...
                        <option value="alta" <?php if($priority == 'alta') { echo 'selected'; } ?>>Alta</option>
 
                        <option value="media" <?php if($priority == 'media') { echo 'selected'; } ?>>Media</option>
    
                        <option value="baja" <?php if($priority == 'baja') { echo 'selected'; } ?>>Baja</option>
       ...
       <div class = "row">
               <div class = "col s12">
                   <p>
                     <input id = "interna" type = "radio" name = "type"
                        value = "Laboratorio" <?php if($type == 'Laboratorio') { echo 'checked'; } ?> />
                     <label for = "interna">Laboratorio</label>
                  </p>

                  <p>
                     <input id = "externa" type = "radio" name = "type"
                        value = "Terreno" <?php if($type == 'Terreno') { echo 'checked'; } ?>/>
                     <label for = "externa">Terreno</label>
                  </p>
                  <p>
                     <input id = "remota" type = "radio" name = "type"
                        value = "Remota" <?php if($type == 'Remota') { echo 'checked'; } ?> />
...
<div class="row">
          ...
          <label for="status">Estado</label>
                  <select  class="browser-default" name="status"  >
                        ...
                        <option value="iniciada" <?php if($status == 'iniciada') { echo 'selected'; } ?>>iniciada</option>
...
$("#subline option:selected").each(function () {
   id_subline = $(this).val();
   $.post("data_subline_equipment.php", {id_subline: id_subline, id_branch: id_branch
   }, function(data) {
         $("#equipment").html(data);
   });
});
...
$("#subline10 option:selected").each(function () {
   ...
   $.post("data_line.php", {
      id_service: id_service, id_branch: id_branch 
         }, function(data) {
   ...
   $.post("data_service.php", {id_service: id_service
         }, function(data) {
         ...
         $.post("data_subline.php", {id_service: id_service 
         }, function(data) {
...
$(document).ready(function () {
   $("#service").change( function () {
      ...
      $("#service option:selected").each(function () {
         ...
         $.post("data_line.php", {id_service: id_service, id_branch: id_branch 
         }, function(data) {
            $("#equipment").html(data);
               $("#equipment2").html(data);
               $("#equipment3").html(data);
         ...
         $.post("data_equipment_show.php", {id_service: id_service
         }, function(data) {
